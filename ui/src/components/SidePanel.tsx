  ...
  return (
    <aside className="side" data-testid="side-panel">
      ...
      <div className="hd"><div className="av" data-state={agent.state}>{role?.avatar ?? "🤖"}</div>
        <div><div className="name">{agent.displayName} — {agent.role}</div><div className="repo">{agent.repo}{a ? ` · #${a.id}` : ""}</div>{agent.resumeSessionId && <div className="repo">🔗 continues session {agent.resumeSessionId.slice(0, 8)}… <button className="btn sm" title="Copy session id" onClick={() => void navigator.clipboard?.writeText(agent.resumeSessionId!)}>⧉</button> <button className="btn sm" title="Rename session" onClick={async () => { const t = window.prompt("Session name"); if (t?.trim()) await api.renameSession(agent.resumeSessionId!, t.trim()).catch(() => {}); }}>✎</button>{live && <span className="st-live"> · live in {live.kind === "background" ? "background" : "terminal"} ({live.status})</span>}</div>}</div></div>
      {a && <>
